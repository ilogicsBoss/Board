...
DATE_TIME::DATE_TIME(const char* date, const char* time) {
	...
	char buff[4] = { 0 };                       // [FIX] const 제거 (sscanf 기록 대상)
	int y;
	int yd, yhh, ymm, yss;                      // [FIX] uint8_t 필드에 %d로 바로 쓰면
	                                            //       인접 멤버가 덮어써져 날짜/시간이 깨짐
	sscanf(date, "%3s %d %d", buff, &yd, &y);   // [FIX] %s에 폭 제한(%3s) 추가
	d = (uint8_t)yd;
	yOff = y >= 2000 ? y - 2000 : y;
	m = (strstr(month_names, buff) - month_names) / 3 + 1;
	sscanf(time, "%d:%d:%d", &yhh, &ymm, &yss);
	hh = (uint8_t)yhh;
	mm = (uint8_t)ymm;
	ss = (uint8_t)yss;
}
